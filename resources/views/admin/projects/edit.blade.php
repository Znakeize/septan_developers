...
<div id="ai-panel" style="position: fixed; top: 0; right: 0; height: 100%; width: 320px; background: linear-gradient(135deg, rgba(46,16,101,0.95), rgba(0,0,0,0.95)); border-left: 1px solid rgba(168,85,247,0.3); backdrop-filter: blur(8px); transform: translateX(100%); transition: transform 0.3s ease; z-index: 1000; color: #e5e7eb;">
    ...
    <div style="padding: 12px;">
        ...
        <div id="ai-tip" style="margin-top:16px; font-size:12px; color:#c4b5fd; border:1px solid rgba(147,51,234,0.25); background: rgba(147,51,234,0.08); border-radius:8px; padding:10px;">Tip: Use existing type and location for better suggestions.</div>
    ...
    <script>
        ...
        function initFeatureCards(){
            // Show cards that already have data, hide the rest except first empty
            ...
            featureCards.forEach((card) => {
                ...
                if (isCardEmpty(card)) {
                    if (!firstEmptyShown) { card.style.display = ''; firstEmptyShown = true; } else { card.style.display = 'none'; }
                } else {
                    card.style.display = '';
                }
            });
